...
class systolic_compute_is:
    ...
    def create_ifmap_prefetch_mat(self):
        ...
        for fc in range(self.col_fold):
            ...
            if delta > 0:
                ...
                this_fold_prefetch = np.concatenate(
                    (this_fold_prefetch, null_req_mat), axis=1
                )
            ...
            if fc == 0:
                ...
                self.ifmap_prefetch_matrix = np.concatenate(
                    (self.ifmap_prefetch_matrix, this_fold_prefetch), axis=0
                )
        ...
        for fr in range(self.row_fold):
            ...
            if fr == 0:
                ...
                self.filter_prefetch_matrix = np.concatenate(
                    (self.filter_prefetch_matrix, this_fold_prefetch), axis=0
                )
    ...
    def create_demand_matrices(self):
        ...
        assert (
            self.ifmap_demand_matrix.shape[0] == self.filter_demand_matrix.shape[0]
        ), "IFMAP and Filter demands out of sync"
        assert (
            self.ofmap_demand_matrix.shape[0] == self.filter_demand_matrix.shape[0]
        ), "OFMAP and Filter demands out of sync"
        assert (
            self.ifmap_demand_matrix.shape[1] == self.arr_col
        ), "IFMAP demands exceed the rows"
        assert (
            self.filter_demand_matrix.shape[1] == self.arr_row
        ), "Filter demands exceed the cols"
        assert (
            self.ofmap_demand_matrix.shape[1] == self.arr_col
        ), "OFMAP demands exceed the cols"
    ...
    def create_ifmap_demand_mat(self):
        ...
        for fc in range(self.col_fold):
            for fr in range(self.row_fold):
                ...
                this_fold_demand = self.ifmap_op_mat_trans[
                    row_start_id:row_end_idx, col_start_id:col_end_idx
                ]
                self.ifmap_reads += (
                    this_fold_demand.shape[0] * this_fold_demand.shape[1]
                )
                ...
                if col_delta > 0:
                    ...
                    this_fold_demand = np.concatenate(
                        (this_fold_demand, null_req_mat), axis=1
                    )

                if row_delta > 0:
                    null_req_mat = np.ones((row_delta, self.arr_col)) * -1
                    this_fold_demand = np.concatenate(
                        (this_fold_demand, null_req_mat), axis=0
                    )
                ...
                cycles_this_fold = (
                    this_fold_demand.shape[0] + this_fold_demand.shape[1] - 1
                )
                compute_cycles_this_fold = mac_used * self.T
                compute_util_this_fold = compute_cycles_this_fold / (
                ...
                if fr == 0 and fc == 0:
                    ...
                    self.ifmap_demand_matrix = np.concatenate(
                        (self.ifmap_demand_matrix, this_fold_demand), axis=0
                    )
            ...
            for fr in range(self.row_fold):
                ...
                if fr == 0 and fc == 0:
                    ...
                    self.filter_demand_matrix = np.concatenate(
                        (self.filter_demand_matrix, this_fold_demand), axis=0
                    )
