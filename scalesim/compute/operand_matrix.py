...
class operand_matrix(object):
    ...
    def set_params(
        ...
        self.ifmap_rows, self.ifmap_cols = self.topoutil.get_layer_ifmap_dims(
            self.layer_id
        )
        self.filter_rows, self.filter_cols = self.topoutil.get_layer_filter_dims(
            self.layer_id
        )
        ...
        self.row_stride, self.col_stride = self.topoutil.get_layer_strides(
            self.layer_id
        )
        # TODO: Marked for cleanup
        ...
        self.ofmap_rows, self.ofmap_cols = self.topoutil.get_layer_ofmap_dims(
            self.layer_id
        )
        ...
        self.ifmap_addr_matrix = np.ones(
            (self.ofmap_px_per_filt * self.batch_size, self.conv_window_size),
            dtype=">i4",
        )
        self.filter_addr_matrix = np.ones(
            (self.conv_window_size, self.num_filters), dtype=">i4"
        )
        self.ofmap_addr_matrix = np.ones(
            (self.ofmap_px_per_filt, self.num_filters), dtype=">i4"
        )
    ...
    def create_ifmap_matrix(self):
        ...
        for row_idx in range(self.batch_size * self.ofmap_px_per_filt):
            for col_idx in range(self.conv_window_size):
                self.ifmap_addr_matrix[row_idx][col_idx] = self.calc_ifmap_elem_addr(
                    i=row_idx, j=col_idx
                )
        return 0
    ...
    def calc_ifmap_elem_addr(self, i, j):
        ...
        c_ch = int(k % channel)
        if (
            c_row + i_row >= self.ifmap_rows or c_col + i_col >= self.ifmap_cols
        ):  # for padded address
            ifmap_px_addr = -1
        else:
            internal_address = (
                c_row * (ifmap_cols * channel) + c_col * channel + c_ch
            )  # Address inside conv window
    ...
    def create_ofmap_matrix(self):
        ...
        for row_idx in range(self.ofmap_px_per_filt):
            for col_idx in range(self.num_filters):
                self.ofmap_addr_matrix[row_idx][col_idx] = self.calc_ofmap_elem_addr(
                    i=row_idx, j=col_idx
                )
        return 0
    ...
    def create_filter_matrix(self):
        ...
        for row_idx in range(self.conv_window_size):
            for col_idx in range(self.num_filters):
                self.filter_addr_matrix[row_idx][col_idx] = self.calc_filter_elem_addr(
                    i=row_idx, j=col_idx
                )
    ...
    def get_ifmap_matrix_part(self, start_row=0, num_rows=-1, start_col=0, num_cols=-1):
        ...
        if not self.matrices_ready_flag:
            if self.params_set_flag:
                ...
                message = (
                    err_prefix + ": Parameters not set yet. Run set_params(). Exiting!"
                )
                print(message)
    ...
    def get_filter_matrix_part(
        self, start_row=0, num_rows=-1, start_col=0, num_cols=-1
    ):
...
if __name__ == "__main__":
    ...
    for i in range(tutil.get_num_layers()):
        ...
        ofmap_px_filt = tutil.get_layer_num_ofmap_px(
            layer_id=i
        ) / tutil.get_layer_num_filters(layer_id=i)
        conv_window_size = tutil.get_layer_window_size(layer_id=i)
        layer_calc_hyper_param_arr = [
            ofmap_dims[0],
            ofmap_dims[1],
            ofmap_px_filt,
            conv_window_size,
